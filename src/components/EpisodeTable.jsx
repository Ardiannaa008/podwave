...
export default function EpisodeTable({ episodes, onDelete }) {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = episodeList.filter((episode) => {
      ...
      const topic = typeof episode.topic === 'string' ? episode.topic : '';
    ...
    return sorted;
  }, [episodes, query, sortKey, sortDir]);
  ...
  return (
    <div className="panel">
      ...
      {rows.length === 0 ? (
        <p className="muted">No episodes match yet. Generate one from the Create page.</p>
      ) : (
